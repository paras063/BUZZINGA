...
exports.getItems = async (req, res) => {
  const items = await Product.find(
    ...
    { name: 1, stock: 1, price: 1, size: 1, color: 1 }
  ).lean();
  res.status("200").json({
    success: true,
    items,
...
exports.viewInvoices = async (req, res) => {
  const invoices = await Invoice.find({ firmId: req.user.firms })
    .lean()
    .populate("firmId sellerId customerId")
    .exec();
...
exports.printInvoice = async (req, res) => {
  ...
  .populate("sellerId")
  .exec();
  res.render("admin/printInvoice", { pageTitle: "Print invoice", invoice });
};
...
exports.mailInvoice = async (req, res) => {
  ...
  const invoice = await Invoice.findById(invoiceId)
    .lean()
    .populate("firmId customerId sellerId")
    .exec();
                              ...
                              <table>
                                  <tr>
                                      ...
                                      <td>
                                          ...
                                          Created: ${
                                            invoiceDetail.invoiceDate.getDate() +
                                            "-" +
                                            invoiceDetail.invoiceDate.getMonth() +
                                            "-" +
                                            invoiceDetail.invoiceDate.getFullYear()
                                          }<br />
                                          Due: ${
                                            invoiceDetail.dueDate.getDate() +
                                            "-" +
                                            invoiceDetail.dueDate.getMonth() +
                                            "-" +
                                            invoiceDetail.dueDate.getFullYear()
                                          }
                                      </td>
                                  </tr>
                              </table>
                          ...
                          <td colspan="2">
                              <table>
                                  <tr>
                                      ...
                                      <td>
                                      <h3>Bill to:-</h3>
                                      ${invoiceDetail.customerId.firstName} ${
        invoiceDetail.customerId.lastName
      }<br/>
                                          ${
